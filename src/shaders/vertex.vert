...
void main(){
    ...
    vec3 normal = (cross(vec3(.0, cz, dz), vec3(dx, cx, 0.)));

    gl_Position = u_projection * u_view * u_model * position;

    v_normal = mat3(u_model) * normal;
    //v_surfaceToLight = u_lightModelPosition - (position * u_model).xyz
    v_surfaceToLight = -lightDirection;
    v_surfaceToView = (-u_view[2] - position).xyz;
}